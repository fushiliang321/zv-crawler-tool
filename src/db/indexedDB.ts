...
export type tableConfigItem = { keyPath?: string, options?: IDBIndexParameters }
export type tableConfig = string[] | Record<string, tableConfigItem>

//创建表
function createTable(db: IDBDatabase, tableName: string, tableConfig: tableConfig) {
...
export default class indexedDBClass {
    _initFinishCallBackFuns: ((value?: unknown) => void)[] = [] //初始化完成回调
    _isInitFinish = false //是否初始化完成
    _db: IDBDatabase | undefined //数据库
    _dbName: string //数据库名
    _dbversion: number //数据库版本
    _tableMap: Record<string, tableConfig> = {} //表配置

    // 构造函数
    constructor(dbName = 'dbName', tableMap = {} ) {
        this._dbName = dbName
        this._dbversion = 1
    ...
    initDB() {
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = event => {
                ...
                // request.result.transaction().objectStore().openCursor()
