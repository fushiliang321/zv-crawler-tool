import IDBProxy, {  AsyncIDBObjectStore } from "./IDBProxy"
...
export default class Table{
    ...
    constructor(db: indexedDBClass, tableName = 'tableName') {
        this.db = db
        this.tableName = tableName
    }

    async store(mode?: IDBTransactionMode): Promise<AsyncIDBObjectStore | undefined> {
     ...
     * @returns
     */
    async putByIndex(indexKey: string, indexValue:  IDBValidKey | IDBKeyRange, data: any, replace: boolean): Promise<IDBValidKey | false> {
        const store = await this.store('readwrite')
        if (!store) {
            return false
        }
    ...
    // 查询数据
    async getAll<T extends any>(query?: IDBValidKey | IDBKeyRange, count?: number): Promise<T[]> {
    ...
    // 通过主键读取数据
    async get<T extends any>(query: IDBValidKey | IDBKeyRange): Promise<T | undefined> {
        return await (await this.store())?.get(query)
    }

    // 通过索引读取数据
    async getByIndex<T extends any>(key: string, query: IDBValidKey | IDBKeyRange): Promise<T | undefined> {
        return (await (await this.store())?.index(key).get(query))
    }

    // 读取索引全部数据
    async getAllByIndex<T extends any>(key: string, query?: IDBValidKey | IDBKeyRange, count?: number): Promise<T[]> {
    ...
    // 通过索引删除数据
    async deleteByIndex(indexKey: string, query?: IDBValidKey | IDBKeyRange, count?: number): Promise<boolean> {
        const store = await this.store('readwrite')
        if (!store) {
            return false
        }
